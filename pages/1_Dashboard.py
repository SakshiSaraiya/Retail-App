...
with h3:
    st.warning(f"Sales Trend: {'Increase' if change >= 0 else 'Decrease'} of {abs(change)} units vs last 7 days")
...
if not low_stock.empty:
    st.error(f"{len(low_stock)} product(s) are low on stock.")
...
if not category_grouped.empty:
    category_fig = px.bar(category_grouped, x='category', y='quantity_sold',
                          title="Category-wise Sales", color='quantity_sold', template='plotly_white')
